...
for name in ['README.md','SECURITY.md','LIMITATIONS.md','LICENSE','THIRD-PARTY.md','package.json','package-lock.json','media/CMakeLists.txt','penguin-stream','penguin-stream.bat','setup.bat','setup.sh']:
    p = root/name
...
with tarfile.open(out,'w') as tar:
    for name,data in sorted(items.items()):
        ...
        entry.mode=0o755 if name in ['media/build/ps-media','scripts/package.py','setup.sh','penguin-stream'] else 0o644
