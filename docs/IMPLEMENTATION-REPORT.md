...
Security highlights: reject unsigned post-MESSAGE-INTEGRITY TURN attrs and duplicate security/scalar attrs, allocation owner checks, expiry checks, capped nonce/allocation resources and pending-bind reservation; UI WS Host/Origin + bounds, strict approval boolean/POST mutations and no-store/referrer; removed automatic unverified viewer trust; input off by default; CLI --yes only synthetic; no silent synthetic fallback. Original report: 40-bit invitation issue subsequently fixed by parent with 160-bit invitations. Residual risks include unaudited custom Noise implementation, no production rendezvous DoS/quotas or coturn deployment, native-decoder hardening and complete shutdown behavior.
