...
1. **40-bit share codes permit offline guessing.** The room identifier is a fast public SHA-256-derived verifier, so a malicious rendezvous can brute-force the code offline; online rate limits do not fix this. Signaling confidentiality must not be advertised as secure against a determined server. Replace with a reviewed PAKE and appropriate rate controls, or a high-entropy invitation link/QR. Human SAS verification remains essential; stored trust is not unattended authorization.
